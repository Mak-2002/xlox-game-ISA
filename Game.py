...
class Game:
    ...
    def main(self):
        welcome_str = ""
        welcome_str += yellow_color + 'An assignment for the Intelligent Search Algorithms course at university, applying concepts to the XLOX game.\n'
        welcome_str += "You can check the rules and play the game at: " + cyan_color + "https://www.puzzleplayground.com/xlox" + yellow_color
        welcome_str += '\n-------------------------------------------'+ reset_color
        print(welcome_str)
        ...
        while True:
            try:
                ...
                if choice == 'u':
                    ...
                    raise Exception
                print(yellow_color + f"New game started in {mode_str} mode..." + reset_color)
                game.play(mode)
            except Exception:
    ...
    def solution_stats(self, node : 'Node', iters : int, print_solution : bool = False):
        ...
        if print_solution:
            ...
            for node in solution:
                stats_str += cyan_color + '#' + str(node.depth + 1) + '\n' + reset_color
    ...
    def play_in_user_mode(self):
        ...
        while not current_node.state.is_final():
            ...
            print(grey_color + '(row) (column) : Make a move  ||  q : terminate\n' + reset_color)
            ...
            if inputs[0] == 'q':
                # End the game
                print(red_color + f"Game terminated with {current_state.get_white_cells_count()} remaining white cells and {moves_count} moves." + reset_color)
                return
            else:
                # Make a move
                try:
        ...
        print( green_color + f"Congrats! You solved the puzzle with {current_node.depth} moves." + reset_color)           
